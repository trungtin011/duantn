...
class AdClickStatsController extends Controller
{
    ...
    /**
     * Trừ phí quảng cáo dựa trên bid_amount của từng chiến dịch
     */
    public function settle(Request $request)
    {
        ...
        try {
            DB::transaction(function () use ($shopId, $startDate, $identityExpr, &$charged, &$skipped, &$totalAmount) {
                ...
                $clicksToCharge = AdClick::from('ad_clicks as ac')
                    ...
                    ->with(['adsCampaign']) // Load thông tin chiến dịch để lấy bid_amount
                    ->orderBy('ac.id')
                    ->get(['ac.*']);

                foreach ($clicksToCharge as $click) {
                    // Lấy bid_amount từ chiến dịch
                    $bidAmount = $click->adsCampaign->bid_amount ?? 1.00;
                    ...
                    $tx = WalletTransaction::create([
                        ...
                        'direction' => 'out',
                        'type' => 'advertising',
                        'description' => "Phí click quảng cáo: {$click->adsCampaign->name} (Giá thầu: {$bidAmount} VNĐ)",
                        'status' => 'completed',
                        'reference_id' => $click->id,
                        'reference_type' => 'ad_click'
                    ]);

                    // Đánh dấu đã charge với cost_per_click thực tế
        ...
        return redirect()->back()->with('success', "Đã trừ phí {$charged} click với tổng số tiền: " . number_format($totalAmount, 0) . " VNĐ. Hết tiền/skip: {$skipped}.");
    }
