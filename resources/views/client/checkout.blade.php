                            ...
                            <form class="space-y-4" action="{{ route('account.addresses.store') }}" method="POST">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input type="text" name="receiver_name" class="w-full bg-gray-50 border border-gray-300 rounded-lg py-2 px-3 focus:outline-none focus:ring-2 focus:ring-primary">
                                    </div>
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 mb-1">
                                            Số điện thoại <span class="text-red-500">*</span>
                                        </label>
                                        <input type="text" name="receiver_phone" class="w-full bg-gray-50 border border-gray-300 rounded-lg py-2 px-3 focus:outline-none focus:ring-2 focus:ring-primary">
                                    </div>
                                </div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    <div class="ghn-address-container">
                                        ...
                                        <select name="province_id" class="w-full bg-gray-50 border border-gray-300 rounded-lg py-2 px-3 focus:outline-none focus:ring-2 focus:ring-primary" id="city">
                                    ...
                                    <div class="ghn-address-container">
                                        ...
                                        <select name="district_id" class="w-full bg-gray-50 border border-gray-300 rounded-lg py-2 px-3 focus:outline-none focus:ring-2 focus:ring-primary" id="district" disabled>
                                    ...
                                    <div class="ghn-address-container">
                                        ...
                                        <select name="ward_id" class="w-full bg-gray-50 border border-gray-300 rounded-lg py-2 px-3 focus:outline-none focus:ring-2 focus:ring-primary" id="ward" disabled>
                                ...
                                <div>
                                    ...
                                    <input type="text" name="address" class="w-full bg-gray-50 border border-gray-300 rounded-lg py-2 px-3 focus:outline-none focus:ring-2 focus:ring-primary">
                                </div>
                                ...
                                <div class="flex gap-3">
                                    <button type="button" class="flex-1 bg-gray-200 text-gray-800 py-2 px-4 rounded-lg hover:bg-gray-300 transition-colors" id="cancelAddressForm">Hủy bỏ</button>
                                    <button type="submit" class="flex-1 bg-gray-600 text-white py-2 px-4 rounded-lg hover:bg-primary-dark transition-colors">Thêm địa chỉ</button>
                                </div>
                            </form>
                        </div>
                    </section>
                                ...
                                <div class="bg-white rounded-xl overflow-hidden shadow-sm">
                                    ...
                                    <div class="w-full flex flex-col px-4 py-2 bg-gray-100 border-t">
                                        <div class="flex flex-col md:flex-row justify-between items-start md:items-center">
                                            ...
                                            <div class="flex flex-col items-center justify-center">
                                                ...
                                                <button type="button" class="bg-gray-400 text-white px-4 py-1 rounded-md hover:bg-accent-dark transition whitespace-nowrap hidden"
                                                        id="cancel-coupon-btn-shop-{{ $shopData['shop']->id }}"
                                                        data-shop-id="{{ $shopData['shop']->id }}"
                                                        >
                                                    Hủy mã
                                                </button>
                                            <button type="button" class="bg-black text-white px-4 py-1 rounded-md hover:bg-accent-dark transition whitespace-nowrap"
                                    ...
                                    <div class="p-6 w-full">
                                        @foreach ($shopData['items'] as $index => $item)
                                            @php
                                                $imagePath = optional($item['product']->images->first())->image_path ?? 'product_images/default.png';
                                            @endphp
                                            <div class="product-card flex p-5 flex-row gap-2 items-center py-1 {{ $index < count($shopData['items']) - 1 ? 'border-b border-gray-200' : '' }}">
                                                <div class="aspect-square w-24 h-24 pr-2 flex-shrink-0">
                                                    <img src="{{ asset('storage/' . $imagePath) }}"
                                                        alt="{{ $item['product']->name }}"
                                                        class="w-full h-full object-contain rounded-lg" style="aspect-ratio: 1 / 1;">
                                                </div>
                                                <div class="w-2/3">
                                                    <div class="text-left">
                                                    ...
                                                    @php
                                                        ...
                                                        } else if ($item['product']->variants && $item['product']->variants->count() > 0) {
                                                    ...
                                                    <div class="flex flex-row justify-between items-end mt-5 gap-2">
                                                        ...
                                                        <div class="text-xs text-green-600 text-right min-w-[110px] mr-10">
                                                            Tổng: <strong>{{ number_format($totalPrice, 0, ',', '.') }}₫</strong>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        ...
                                        <div class="mt-4">
                                            ...
                                            <textarea name="note_for_shop_{{ $shopData['shop']->id }}"
                                                      data-shop-id="{{ $shopData['shop']->id }}"
                                ...
                                <div id="discounts-modal-{{ $shopData['shop']->id }}" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden z-50">
                                    <div class="relative top-20 mx-auto p-5 border w-11/12 md:w-3/4 lg:w-1/2 shadow-lg rounded-md bg-white">
                                        <div class="mt-3 text-center">
                                            <h3 class="text-lg font-medium text-gray-900">Mã giảm giá của Shop: {{ $shopData['shop']->shop_name }}</h3>
                    ...
                    <section class="bg-gray-50 p-6 rounded-xl">
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="flex items-center gap-2 mt-2 hidden" id="used-points-input-group">
                                ...
                                <input type="number" id="used_points" name="used_points" min="0" max="{{ $user_points }}"
                                       step="100" value="0"
                    ...
                    <section class="summary-card p-6">
                        ...
                        <div class="space-y-3 mb-4">
                            ...
                            <div class="flex justify-between text-gray-600 discount-row-shop" style="display:none">
                                ...
                                <span class="text-green-600" id="discount_shop_fee">{{ number_format(0, 0, ',', '.') }}₫</span>
                            </div>
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            if(data.used_coupon_data.type_coupon == 'shipping') {
                                document.getElementById('discount_shipping_fee').innerText = data.used_coupon_data.discount_value.toLocaleString('vi-VN') + '₫';
                            }else{
                                let discountAmount = document.getElementById('discount-amount-shop-' + shopId);
                                discountAmount.innerText = data.used_coupon_data.discount_value.toLocaleString('vi-VN') + '₫';
                                discountAmount.parentElement.classList.remove('hidden');

                                document.querySelector('.discount-row-shop').style.display = 'flex';
                                document.getElementById('discount_shop_fee').innerText = calculateDiscountTotal().toLocaleString('vi-VN') + '₫';

                                let cancelBtn = document.getElementById('cancel-coupon-btn-shop-' + shopId);
            ...
            window.addEventListener('DOMContentLoaded', (e) => {
                document.querySelectorAll('[id^="cancel-coupon-btn-shop-"]').forEach(btn => {
                    btn.addEventListener('click', (e) => {
                        ...
                        document.getElementById('cancel-coupon-btn-shop-' + shopId).classList.add('hidden');
                        const discountAmount = document.getElementById('discount-amount-shop-' + shopId);
                        if (discountAmount) {
                        ...
                        if(calculateDiscountTotal() == 0) {
                            document.querySelector('.discount-row-shop').style.display = 'none';
                        }
                        updateTotal();
