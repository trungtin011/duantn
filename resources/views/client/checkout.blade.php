                        ...
                        <div class="space-y-5">
                            @foreach ($groupedItems as $shopId => $shopData)
                                ...
                                <div id="discounts-modal-{{ $shopData['shop']->id }}"
                                    class="fixed top-[150px] inset-0 overflow-y-auto h-full w-full hidden z-50">
                                    <div class="relative top-10 mx-auto p-0 border-0 w-11/12 md:w-4/5 lg:w-3/4 xl:w-2/3 shadow-lg rounded-xl bg-white overflow-hidden">
                                        <!-- Header -->
                                        <div class="bg-gray-300 px-6 py-4 border-b">
                                            <div class="flex justify-between items-center">
                                                <div class="flex items-center space-x-3">
                                                    ...
                                                    <div>
                                                        <h3 class="text-xl font-semibold text-gray-800">Mã giảm giá của Shop</h3>
                                                        <p class="text-gray-600 text-sm">{{ $shopData['shop']->shop_name }}</p>
                                                    </div>
                                                </div>
                                                <button onclick="closeDiscountsModal({{ $shopData['shop']->id }})"
                                                    class="w-8 h-8 bg-gray-200 hover:bg-gray-300 rounded-full flex items-center justify-center transition-all duration-200">
                                        ...
                                        <div class="p-6">
                                            @if ($shop_coupons[$shopData['shop']->id] && $shop_coupons[$shopData['shop']->id]->count() > 0)
                                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-96 overflow-y-auto">
                                                    @foreach ($shop_coupons[$shopData['shop']->id] as $coupon)
                                                        <div class="bg-white border border-gray-200 rounded-xl p-4 hover:border-primary hover:shadow-sm transition-all duration-200 relative">
                                                            <!-- Coupon Type Badge -->
                                                            ...
                                                            <div class="text-center mb-3">
                                                                <div class="bg-gray-50 rounded-lg p-3 mb-2">
                                                                    <h4 class="font-mono font-bold text-lg text-gray-800 tracking-wider">{{ $coupon->code }}</h4>
                                                                </div>
                                                            </div>

                                                            <!-- Coupon Details -->
                                                            <div class="space-y-2 mb-4">
                                                                <p class="text-gray-700 text-sm leading-relaxed text-center">{{ $coupon->description }}</p>
                                                                
                                                                <!-- Discount Amount -->
                                                                <div class="flex items-center justify-center space-x-2">
                                                                    <span class="text-gray-500 text-sm">Giảm:</span>
                                                                    @if ($coupon->discount_type == 'percentage')
                                                                        <span class="text-xl font-bold text-green-600">{{ $coupon->discount_value }}%</span>
                                                                    @else
                                                                        <span class="text-xl font-bold text-green-600">{{ number_format($coupon->discount_value, 0, ',', '.') }}₫</span>
                                                                    @endif
                                                                </div>
                                                            ...
                                                            <!-- Use Button -->
                                                            <button type="button"
                                                                class="use-shop-coupon-btn w-full bg-gradient-to-r from-green-500 to-green-600 text-white py-2 px-4 rounded-lg font-medium hover:from-green-600 hover:to-green-700 transition-all duration-200 shadow-sm hover:shadow-md"
                                                                data-shop-id="{{ $shopData['shop']->id }}"
                                            ...
                                            @else
                                                <div class="text-center py-12">
                                                    ...
                                                    <h4 class="text-lg font-semibold text-gray-600 mb-2">Không có mã giảm giá</h4>
                                                    <p class="text-gray-500 text-sm">Shop này hiện không có mã giảm giá nào khả dụng.</p>
                                                </div>
                                            @endif
                                        </div>
                                        ...
                                        <div class="bg-gray-50 px-6 py-4 border-t">
                                            <div class="flex justify-between items-center">
                                                ...
                                                <div class="flex space-x-3">
                                                    <button onclick="closeDiscountsModal({{ $shopData['shop']->id }})"
                                                        class="px-4 py-2 bg-gray-300 text-gray-700 rounded-lg hover:bg-gray-400 transition-colors duration-200 text-sm">
                                                        Đóng
                                                    </button>
                                                </div>
                                            </div>
...
@push('scripts')
    ...
    <script>
        ...
        function applyPlatformDiscount(code, discountType, discountValue, couponType) {
            ...
            updateTotal();
                                            showSuccessMessage('Áp dụng mã giảm giá của sàn thành công!');
        }
                    ...
                    fetch('{{ route('customer.apply-shop-discount') }}', {
                        ...
                        .then(data => {
                            if (data.success) {
                                ...
                                showSuccessMessage('Áp dụng mã giảm thành công!');
                            } else {
                        ...
                        .catch(error => {
                            ...
                            showErrorMessage('Có lỗi xảy ra khi áp dụng mã giảm');
                        });
                });
            });
    ...
    <script>
        ...
        function showSuccessMessage(message) {
            ...
            toast.className = 'fixed inset-0 flex items-center justify-center z-[9999]';
            
            const toastContent = document.createElement('div');
            toastContent.className = 'bg-green-500 text-white px-8 py-6 rounded-xl shadow-2xl flex items-center max-w-md mx-4 transform transition-all duration-300 scale-100';
            ...
            setTimeout(() => {
                toastContent.classList.add('scale-65', 'opacity-0');
                setTimeout(() => {
                    toast.remove();
                }, 300);
            }, 3000);
        ...
        function showErrorMessage(message) {
            ...
            const toastContent = document.createElement('div');
            toastContent.className = 'bg-red-500 text-white px-8 py-6 rounded-xl shadow-2xl flex items-center max-w-md mx-4 transform transition-all duration-300 scale-100';
            toastContent.innerHTML = `
                <i class="fas fa-exclamation-circle mr-4 text-3xl"></i>
                <span class="text-lg font-medium">${message}</span>
            `;
