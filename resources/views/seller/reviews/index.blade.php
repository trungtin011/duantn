...
<div class="max-w-7xl mx-auto p-4 sm:p-6 md:p-8">
    <header class="flex justify-between items-center border-b border-gray-200 pb-3 mb-6">
        <div>
    ...
    <!-- Form lọc -->
    <form action="{{ route('seller.reviews.index') }}" method="GET" class="flex flex-wrap gap-x-4 gap-y-3 items-center mb-6 text-xs text-gray-700">
        <label class="flex items-center gap-2 whitespace-nowrap">
            <span class="min-w-[80px]">Tên sản phẩm:</span>
        ...
        <label class="flex items-center gap-2 whitespace-nowrap">
            <span class="min-w-[80px]">Phân loại hàng:</span>
        ...
        <label class="flex items-center gap-2 whitespace-nowrap">
            <span class="min-w-[80px]">Người mua:</span>
        ...
        <label class="flex items-center gap-2 whitespace-nowrap">
            <span class="min-w-[80px]">Thời gian:</span>
                    ...
                    <tr class="border-t">
                        <td class="px-3 py-2">
                            <div class="flex items-start gap-2">
                                <img src="{{ $review->product->thumbnail_url ?? '/default.jpg' }}"
                                    class="w-10 h-10 object-cover rounded" />
                                <div class="leading-snug text-xs">
                                    <div class="font-semibold truncate max-w-[150px]">{{ $review->product->name }}</div>
                                    <div class="text-gray-400">Phân loại: {{ $review->variation ?? 'Không rõ' }}</div>
                        ...
                        <td class="px-3 py-2 text-center align-top">
                            <div class="text-gray-500 text-[10px] mb-1">Đơn hàng: {{ $review->order->code ?? '-' }}</div>
