...
<div class="container">
    ...
    <form action="{{ route('seller.combo.store') }}" method="POST">
        @csrf
        <div class="form-group">
            <label for="combo_name">Tên Combo</label>
            <input type="text" name="combo_name" id="combo_name" class="form-control" value="{{ old('combo_name') }}" required>
        </div>
        <div class="form-group">
            <label for="combo_description">Mô tả</label>
            <textarea name="combo_description" id="combo_description" class="form-control">{{ old('combo_description') }}</textarea>
        </div>
        <div class="form-group">
            <label for="total_price">Tổng giá (VNĐ)</label>
            <input type="number" name="total_price" id="total_price" class="form-control" value="{{ old('total_price') }}" readonly>
        </div>
        <div class="form-group">
            ...
            <div class="product-item mb-3">
                <div class="row">
                    <div class="col-md-8">
                        ...
                        <select name="products[0][productID]" class="form-control product-select" required onchange="calculateTotal()">
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="number" name="products[0][quantity]" class="form-control quantity-input" value="1" min="1" required oninput="calculateTotal()">
                    </div>
                </div>
            </div>
        </div>
