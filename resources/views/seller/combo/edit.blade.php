...
@section('content')
    ...
    <section class="bg-white rounded-lg shadow-sm p-6 max-w-3xl">
        ...
        <form action="{{ route('seller.combo.update', $combo->id) }}" method="POST" id="combo-form" class="space-y-6" enctype="multipart/form-data">
            @csrf
            @method('PATCH')
            ...
            <div class="flex flex-col">
                ...
                <input type="text" name="name" id="name"
                    ...
                    value="{{ old('name', $combo->combo_name) }}" required>
                @error('name')
            ...
            <div class="flex flex-col">
                ...
                <textarea name="combo_description" id="combo_description"
                    ...
                    rows="4">{{ old('combo_description', $combo->combo_description) }}</textarea>
            ...
            <div class="flex flex-col">
                ...
                @if ($combo->image)
                    <img src="{{ asset('storage/' . $combo->image) }}" alt="Combo Image" class="w-32 h-32 object-cover mb-2">
                @endif
            ...
            <div class="flex flex-col">
                ...
                <input type="number" name="quantity" id="quantity"
                    class="border border-[#F2F2F6] rounded-md p-2 h-9 text-xs text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-600"
                    value="{{ old('quantity', $combo->quantity) }}" min="0" required>
                @error('quantity')
                    <span class="text-red-600 text-xs">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="flex flex-col">
                ...
                <input type="number" name="total_price" id="total_price"
                    ...
                    value="{{ old('total_price', $combo->total_price) }}" readonly>
                @error('total_price')
                    <span class="text-red-600 text-xs">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="flex flex-col">
                ...
                <input type="number" name="discount_value" id="discount_value"
                    class="border border-[#F2F2F6] rounded-md p-2 h-9 text-xs text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-600"
                    value="{{ old('discount_value', $combo->discount_value) }}" min="0" oninput="calculateTotal()">
                        ...
                        <div class="product-item flex flex-col md:flex-row md:items-end gap-4 relative">
                            <div class="flex-1">
                                ...
                                <select name="products[{{ $index }}][productID]"
                                    ...
                                    required onchange="updateVariants(this, {{ $index }}); calculateTotal()">
                                    <option value="">Chọn sản phẩm</option>
                                    @foreach ($products as $product)
                                        <option value="{{ $product->id }}"
                                            data-price="{{ $product->sale_price ?? $product->price }}"
                                            data-variants="{{ json_encode($product->variants) }}"
                                            {{ old('products.' . $index . '.productID', $comboProduct->productID) == $product->id ? 'selected' : '' }}>
                                            {{ $product->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="flex-1">
                                ...
                                <select name="products[{{ $index }}][variantID]"
                                    ...
                                    @if ($comboProduct->productID && $selectedProduct = $products->firstWhere('id', $comboProduct->productID))
                                        @foreach ($selectedProduct->variants as $variant)
                                            <option value="{{ $variant->id }}"
                                                data-price="{{ $variant->sale_price ?? $variant->price }}"
                                                {{ old('products.' . $index . '.variantID', $comboProduct->variantID) == $variant->id ? 'selected' : '' }}>
                            ...
                            <div class="w-full md:w-[120px]">
                                ...
                                <input type="number" name="products[{{ $index }}][quantity]"
                                    ...
                                    value="{{ old('products.' . $index . '.quantity', $comboProduct->quantity) }}" min="1" required oninput="calculateTotal()">
                            </div>
