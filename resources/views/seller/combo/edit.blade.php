...
@section('content')
    ...
    <section class="bg-white rounded-lg shadow-sm p-6 max-w-3xl">
        ...
        <form action="{{ route('seller.combo.update', $combo->id) }}" method="POST" id="combo-form" class="space-y-6">
            @csrf
            @method('PATCH')
            ...
            <div class="flex flex-col">
                ...
                <input type="text" name="combo_name" id="combo_name"
                    ...
                    value="{{ old('combo_name', $combo->combo_name) }}" required>
            </div>
            ...
            <div class="flex flex-col">
                ...
                <input type="number" name="discount_value" id="discount_value"
                    class="border border-[#F2F2F6] rounded-md p-2 h-9 text-xs text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-600"
                    value="{{ old('discount_value', $combo->discount_value) }}" min="0">
            </div>
            ...
            <div class="flex flex-col">
                ...
                <select name="discount_type" id="discount_type"
                    ...
                    <option value="percentage"
                        {{ old('discount_type', $combo->discount_type) == 'percentage' ? 'selected' : '' }}>%</option>
                    <option value="fixed" {{ old('discount_type', $combo->discount_type) == 'fixed' ? 'selected' : '' }}>
                        VNĐ</option>
                </select>
            </div>
                    ...
                    @foreach ($combo->products as $index => $comboProduct)
                        <div class="product-item flex flex-col md:flex-row md:items-center gap-4 relative group">
                                ...
                                <select name="products[{{ $index }}][productID]"
                                    ...
                                    @foreach ($products as $product)
                                        <option value="{{ $product->id }}" data-price="{{ $product->price }}"
                                            {{ $comboProduct->productID == $product->id ? 'selected' : '' }}>
                                            {{ $product->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <button type="button" onclick="removeProduct(this)"
                                class="text-red-500 text-sm hover:text-red-700 px-2 py-1 mt-4" title="Xoá">X</button>
                        </div>
                    @endforeach
                </div>
                <button type="button"
