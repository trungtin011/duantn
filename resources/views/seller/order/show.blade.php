...
<div class="container mx-auto py-5">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold">Chi tiết đơn hàng #{{ $order->order_code ?? 'N/A' }} - {{ $shop_order->status ?? 'N/A' }}</h1>
        ...
        @if(in_array($shop_order->status, ['confirmed', 'ready_to_pick', 'picked']))
            <form action="{{ route('seller.order.cancel') }}" method="POST">
                ...
                <button class="bg-red-500 text-white px-4 py-2 rounded-md" type="submit">Hủy đơn hàng</button>
            </form>
        @endif
    ...
    <div class="bg-white shadow-sm rounded-lg p-6 mb-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <p><strong>Mã đơn hàng:</strong> {{ $shop_order->code ?? 'N/A' }}</p>
                <p><strong>Ngày đặt:</strong> {{ $order->created_at->format('d/m/Y') ?? 'N/A' }}</p>
                <p><strong>Mã vận đơn:</strong> {{ $shop_order->tracking_code ?? 'N/A' }}</p>
                <p><strong>Lời nhắn của khách hàng:</strong> {{ $shop_order->note ?? 'Không có' }}</p>
            </div>
            <div>
                <h2 class="text-lg font-semibold mb-4">Địa chỉ giao hàng</h2>
                @if($order->address)
                    <p><strong>Tên người nhận:</strong> {{ $order->address->receiver_name ?? 'N/A' }}</p>
                    <p><strong>Số điện thoại:</strong> {{ $order->address->receiver_phone ?? 'N/A' }}</p>
                    ...
                    <tr class="border-b border-gray-200">
                        <td class="py-4 px-4">
                            <div class="flex items-center">
                                ...
                                <div>
                                    <p class="text-gray-700">{{ $item->product_name ?? 'N/A' }}</p>
                                    <p class="text-sm text-gray-500">{{ $item->variant_name ?? 'N/A' }}</p>
                                </div>
                            </div>
                        </td>
                        <td class="py-4 text-center">{{ $item->quantity ?? 0 }}</td>
                        <td class="py-4 text-center">{{ number_format($item->unit_price ?? 0, 0, ',', '.') }}đ</td>
                        <td class="py-4 text-center">{{ number_format($item->total_price ?? 0, 0, ',', '.') }}đ</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
                        ...
                        <div class="ml-7 pl-5 border-l-2 border-blue-100 space-y-4">
                            @foreach ($histories as $history)
                                <div class="relative pb-4">
                                    <div class="absolute -left-5 top-3 h-3 w-3 rounded-full {{ $statusClasses[$history->status] ?? 'bg-gray-200' }} border-2 border-white"></div>
                                    <div class="bg-gray-50 rounded-lg p-4 shadow-sm">
                                        ...
                                        <div class="mt-2 text-sm text-gray-700 space-y-1">
                                            <p><span class="font-medium">Mô tả: </span> {{ $history->description ?? 'N/A' }}</p>
                                            @if ($history->note)
