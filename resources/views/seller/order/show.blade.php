...
<div class="container mx-auto py-8 px-4">
        ...
        <div class="flex flex-wrap gap-3">
            @if($shop_order->status === 'pending')
                <form action="{{ route('seller.order.update-status', ['id' => $shop_order->id]) }}" method="POST">
                    ...
                    <button class="flex items-center bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg transition-all transform hover:scale-105" type="submit">
            ...
            @if(in_array($shop_order->status, ['shipping', 'ready_to_pick']))
                <form action="{{ route('seller.order.tracking') }}" method="POST">
                    ...
                    <button class="flex items-center bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-lg transition-all transform hover:scale-105" type="submit">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <form action="{{ route('seller.order.refund') }}" method="POST">
                    ...
                    <button class="flex items-center bg-orange-600 hover:bg-orange-700 text-white px-4 py-2 rounded-lg transition-all transform hover:scale-105" type="submit">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                        </svg>
                        Trả hàng
            ...
            <div class="p-5">
                <div class="space-y-3">
                    ...
                    <div class="flex justify-between">
                        ...
                        <span class="font-medium text-right">{{ $order->shop_order->first()->note ?? 'Không có' }}</span>
                    </div>
                </div>
            </div>
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Thành tiền</th>
                    </tr>
                </thead>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                <div class="flex-shrink-0 h-16 w-16 bg-gray-100 rounded-md overflow-hidden">
                                    <img class="h-full w-full object-contain" src="{{ $item->product_image ?? 'https://via.placeholder.com/150' }}" alt="{{ $item->product_name }}">
                                </div>
                                <div class="ml-4">
    ...
    <div class="bg-white rounded-xl shadow-md overflow-hidden mb-8 transition-all hover:shadow-lg animate__animated animate__fadeIn">
        ...
        <div class="p-6">
            <form action="{{ route('seller.order.shipping', $order->id) }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                    <div>
                        <label for="shipping_provider" class="block text-sm font-medium text-gray-700 mb-1">Đơn vị vận chuyển</label>
                    ...
                    <div>
                        <label for="required_note" class="block text-sm font-medium text-gray-700 mb-1">Hình thức kiểm hàng</label>
                ...
                <div class="mb-6">
                    <label for="note" class="block text-sm font-medium text-gray-700 mb-1">Ghi chú cho đơn vị vận chuyển</label>
                    <textarea name="note" id="note" rows="3" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"></textarea>
                </div>
                
                <div class="flex justify-end">
                    <button type="submit" class="inline-flex items-center px-6 py-3 border border-transparent text-base font-medium rounded-md shadow-sm text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-all transform hover:scale-105">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                        </svg>
                        Xác nhận gửi hàng
        ...
        @else
            @php
                ...
                $statusIcons = [
                    ...
                    'picked' => 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2',
                    'shipping' => 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2',
                    'delivered' => 'M5 13l4 4L19 7',
                    'cancelled' => 'M6 18L18 6M6 6l12 12',
                    'shipping_failed' => 'M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z',
                        ...
                        @foreach ($groupedStatus as $date => $histories)
                            <li class="mb-8">
                                <div class="flex items-center mb-4">
                                    <span class="flex-shrink-0 h-8 w-8 rounded-full bg-indigo-100 flex items-center justify-center">
                                ...
                                <ul class="space-y-6">
                                    ...
                                    <li class="relative pl-10 pb-6 last:pb-0">
                                        <div class="absolute top-0 left-0 h-full w-0.5 bg-gray-200" aria-hidden="true"></div>
                                        <div class="relative flex space-x-3">
                                            <div>
                                                <span class="h-8 w-8 rounded-full {{ $statusColors[$history->status] ?? 'bg-gray-100 text-gray-800' }} flex items-center justify-center ring-8 ring-white">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $statusIcons[$history->status] ?? 'M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z' }}" />
                                                    </svg>
                                                </span>
                                            </div>
                                            <div class="min-w-0 flex-1 pt-1.5 flex justify-between space-x-4">
                                                <div>
                                                    <p class="text-sm text-gray-500">
                                                        ...
                                                        <span class="font-medium text-gray-900 ml-1">{{ $history->created_at->format('H:i') }}</span>
                                                    </p>
