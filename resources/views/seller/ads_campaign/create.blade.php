    ...
    <section class="bg-white rounded-lg shadow-sm p-6">
        <form action="{{ route('seller.ads_campaigns.store') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="number" step="0.01" min="0" class="form-input w-full border border-gray-300 rounded-md p-2" id="bid_amount" name="bid_amount" value="{{ old('bid_amount', 1.00) }}" required>
                @error('bid_amount')
                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                @enderror
            </div>
