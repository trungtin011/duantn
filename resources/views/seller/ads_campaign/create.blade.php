...
@section('content')
    ...
    <section class="bg-white rounded-lg shadow-sm p-6">
        <form action="{{ route('seller.ads_campaigns.store') }}" method="POST" id="adsCampaignForm" novalidate>
            @csrf
            <div class="mb-4">
                <label for="name" class="block text-gray-700 text-sm font-bold mb-2">Tên chiến dịch:</label>
                <input type="text" class="form-input w-full border border-gray-300 rounded-md p-2" id="name" name="name" value="{{ old('name') }}">
                @error('name')
            ...
            <div class="mb-4">
                ...
                @error('start_date')
                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                @enderror
                <p id="start_date_error" class="text-red-500 text-xs italic hidden"></p>
            </div>
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <input type="number" step="0.01" min="0" max="{{ $walletBalance ?? 0 }}" class="form-input w-full border border-gray-300 rounded-md p-2" id="bid_amount" name="bid_amount" value="{{ old('bid_amount', 1.00) }}">
                @error('bid_amount')
                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                @enderror
    ...
    <script>
        (function() {
            ...
            form.addEventListener('submit', function(e) {
                ...
                // Reset
                ['name_error','start_date_error','end_date_error','bid_amount_error'].forEach(id => err(id, ''));
