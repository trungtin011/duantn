            ...
            <form action="{{ route('admin.notifications.update', $notification->id) }}" method="post"
                ...
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="title" name="title"
                            value="{{ old('title', $notification->title) }}" placeholder="Nhập tiêu đề thông báo" required>
                    ...
                    <div class="form-group">
                        ...
                        <select name="type" id="type" class="form-select">
                            <option value="promotion" {{ $notification->type == 'promotion' ? 'selected' : '' }}>Khuyến mãi
                            </option>
                            <option value="system" {{ $notification->type == 'system' ? 'selected' : '' }}>Hệ thống</option>
                            <option value="security" {{ $notification->type == 'security' ? 'selected' : '' }}>Bảo mật
                            </option>
                        </select>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control" id="content" name="content" rows="5"
                        placeholder="Nhập nội dung chi tiết thông báo" required>{{ old('content', $notification->content) }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <div id="image_preview" style="margin-top: 10px;">
                        @if ($notification->image_path)
                            <img id="preview_img" src="{{ asset('images/notifications/' . $notification->image_path) }}"
                                alt="Xem trước ảnh"
                                style="max-width: 200px; max-height: 200px; border: 1px solid #ddd; border-radius: 4px;" />
                        @else
                            <img id="preview_img" src="#" alt="Xem trước ảnh"
                                style="display: none; max-width: 200px; max-height: 200px; border: 1px solid #ddd; border-radius: 4px;" />
                        @endif
                    </div>
                </div>
                <script>
                ...
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <select name="receiver_type" id="receiver_type" class="form-select">
                            <option value="user" {{ $notification->receiver_type == 'user' ? 'selected' : '' }}>Người dùng
                            </option>
                            <option value="shop" {{ $notification->receiver_type == 'shop' ? 'selected' : '' }}>Cửa hàng
                            </option>
                            <option value="admin" {{ $notification->receiver_type == 'admin' ? 'selected' : '' }}>Quản trị
                                viên</option>
                            <option value="all" {{ $notification->receiver_type == 'all' ? 'selected' : '' }}>Tất cả
                            </option>
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-select" id="priority" name="priority">
                            <option value="low" {{ $notification->priority == 'low' ? 'selected' : '' }}>Thấp</option>
                            <option value="normal" {{ $notification->priority == 'normal' ? 'selected' : '' }}>Bình thường
                            </option>
                            <option value="high" {{ $notification->priority == 'high' ? 'selected' : '' }}>Cao</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="direct_to" name="direct_to"
                        value="{{ old('direct_to', $receiver->receiver_id ?? '') }}"
                        placeholder="Nhập ID mục tiêu cụ thể (nếu có)">
                    <div class="form-note">Nhập ID mục tiêu cụ thể để gửi thông báo trực tiếp (để trống để gửi cho tất cả)
                    </div>
                </div>
