...
@section('content')
    <div class="admin-page-header">
        <h1 class="admin-page-title">Quản lý Danh mục bài viết</h1>
        <div class="admin-breadcrumb"><a href="#" class="admin-breadcrumb-link">Home</a> / Danh mục bài viết</div>
    </div>

    <section class="bg-white rounded-lg shadow-sm p-6">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-4 h-[72px]">
            <form class="w-full md:w-[223px] relative" method="GET" action="{{ route('post-categories.index') }}">
                <input name="search"
        ...
        <table class="w-full text-xs text-left text-gray-400 border-t border-gray-100">
            ...
            <tbody class="divide-y divide-gray-100 text-gray-900 font-normal">
                @foreach ($postCategories as $category)
                    <tr>
                        <td class="py-4 pr-6">
                            <input class="select-item w-[18px] h-[18px]" aria-label="Chọn {{ $category->title }}"
                                type="checkbox" />
                        </td>
                        ...
                        <td class="py-4 text-[13px] text-gray-500">{{ $category->slug }}</td>
                        ...
                        <td class="py-4 text-[13px]">{{ $category->created_at->format('d/m/Y') }}</td>
                        <td class="py-4">
                            <span
                                class="inline-block {{ $category->status == 'active' ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600' }} text-[10px] font-semibold px-2 py-0.5 rounded-md select-none">
                        ...
                        <td class="py-4 pr-6 text-right flex items-center gap-2 justify-end">
                            ...
                            <form action="{{ route('post-categories.destroy', $category->id) }}" method="POST"
                                onsubmit="return confirm('Bạn có chắc muốn xóa danh mục này?')">
                                @csrf
                                @method('DELETE')
                                <button type="submit" aria-label="Xóa {{ $category->title }}"
                ...
                @if ($postCategories->isEmpty())
                    <tr>
                        <td colspan="7" class="text-center text-gray-400 py-4">Không tìm thấy danh mục nào</td>
                    </tr>
                @endif
            </tbody>
        </table>
        <div class="mt-6 flex items-center justify-between text-[11px] text-gray-500 select-none">
            <div>
                Hiển thị {{ $postCategories->count() }} danh mục trên {{ $postCategories->total() }} danh mục
            </div>
