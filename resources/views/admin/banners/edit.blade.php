...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-2xl mx-auto">
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <form action="{{ route('admin.banners.update', $banner->id) }}" method="POST" enctype="multipart/form-data" id="banner-form">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    ...
                    <input type="hidden" name="content_position" id="content_position" value="{{ old('content_position', $banner->content_position) ?? 'center' }}">
                    <input type="hidden" name="text_align" id="text_align" value="{{ old('text_align', $banner->text_align) ?? 'center' }}">
                    <input type="hidden" name="title_color" id="title_color" value="{{ old('title_color', $banner->title_color) ?? '#ffffff' }}">
                ...
                <div class="mb-6">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="color" id="preview-title-color" value="{{ $banner->title_color ?? '#ffffff' }}" class="w-full h-10 border border-gray-300 rounded-md">
                        </div>
                        <div>
                            <label for="preview-subtitle-color" class="block text-sm text-gray-600 mb-1">Màu phụ đề</label>
                            <input type="color" id="preview-subtitle-color" value="{{ $banner->subtitle_color ?? '#f3f4f6' }}" class="w-full h-10 border border-gray-300 rounded-md">
                        </div>
                    </div>
                </div>
                ...
                <div class="mb-6">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" id="preview-title-font-size" value="{{ $banner->title_font_size ?? '2rem' }}" 
                                   class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                                   placeholder="2rem">
                        </div>
                        <div>
                            <label for="preview-subtitle-font-size" class="block text-sm text-gray-600 mb-1">Kích thước phụ đề</label>
                            <input type="text" id="preview-subtitle-font-size" value="{{ $banner->subtitle_font_size ?? '1rem' }}" 
                                   class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                                   placeholder="1rem">
                        </div>
                    </div>
                ...
                <div id="banner-preview" class="relative w-full h-96 bg-gray-100 rounded-lg overflow-hidden">
                    ...
                    <div id="preview-content" class="absolute inset-0 flex items-center justify-center p-8">
                        <div class="text-center">
                            <p id="preview-subtitle" class="text-sm mb-2" style="color: {{ $banner->subtitle_color ?? '#f3f4f6' }};">{{ $banner->description ?? 'Mô tả banner' }}</p>
                            <h2 id="preview-title" class="text-2xl font-bold mb-4" style="color: {{ $banner->title_color ?? '#ffffff' }};">{{ $banner->title }}</h2>
                            <a href="{{ $banner->link_url ?? '#' }}" id="preview-btn-link" class="inline-block px-6 py-2 bg-white text-gray-900 rounded-md hover:bg-gray-100">
                                Xem chi tiết
                            </a>
                        </div>
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('Banner edit preview script loaded');
    ...
    if (previewBtn) {
        previewBtn.addEventListener('click', function(e) {
            ...
            const description = document.getElementById('description')?.value || '{{ $banner->description }}';
            const linkUrl = document.getElementById('link_url')?.value || '{{ $banner->link_url }}';
            ...
            if (imageFile) {
                ...
                previewBannerImg.src = '{{ $banner->image_url }}';
            }
