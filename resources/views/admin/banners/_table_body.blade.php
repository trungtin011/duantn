    ...
    <tr class="hover:bg-gray-50">
        <td class="px-6 py-4 whitespace-nowrap">
            <div class="flex items-center">
                <div class="flex-shrink-0 h-16 w-24">
                    <img class="h-16 w-24 object-cover rounded-lg" src="{{ $banner->image_url }}"
                        alt="{{ $banner->title }}">
                </div>
                <div class="ml-3 text-xs text-gray-500">
                    @if ($banner->image_width && $banner->image_height)
                        <p>{{ $banner->image_width }} x {{ $banner->image_height }} px</p>
                    @endif
                    @if ($banner->image_size)
                        <p>{{ $banner->image_size }}</p>
                    @endif
                </div>
            </div>
        </td>
        ...
        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
            <div class="flex items-center justify-end gap-2">
                ...
                <a href="{{ route('admin.banners.show', $banner->id) }}" class="text-blue-600 hover:text-blue-900"
                    title="Xem chi tiết">
                    <i class="fas fa-eye"></i>
                </a>
                <a href="{{ route('admin.banners.edit', $banner->id) }}" class="text-yellow-600 hover:text-yellow-900"
                    title="Chỉnh sửa">
                    <i class="fas fa-edit"></i>
                </a>
                <button type="button" class="text-red-600 hover:text-red-900 delete-banner"
                    data-banner-id="{{ $banner->id }}" data-banner-title="{{ $banner->title }}" title="Xóa">
