...
<head>
    ...
    <style>
        ...
        /* Tăng kích thước chữ và biểu tượng trong dropdown_admin */
        .dropdown_admin {
            padding: 16px 28px;
            /* Tăng padding để cân đối */
            font-size: 16px;
            /* Tăng kích thước chữ cho tiêu đề menu */
            font-weight: 500;
            /* Làm chữ đậm hơn một chút nếu cần */
        }

        .dropdown_admin svg {
            width: 20px;
            /* Tăng kích thước biểu tượng SVG */
            height: 20px;
        }

        /* Tăng kích thước chữ trong dropdown_products_content */
        .dropdown_products_content {
            padding: 16px 48px;
            /* Tăng padding để cân đối */
        }

        .dropdown_products_content li {
            font-size: 15px;
            /* Tăng kích thước chữ cho các mục trong dropdown */
        }

        /* Tăng kích thước icon chevron */
        .dropdown_admin i.fa-chevron-right {
            font-size: 14px;
        ...
        .dropdown_admin:hover {
            background-color: #f3f4f6;
        }

        /* Kích thước biểu tượng SVG */
        .dropdown_admin svg {
            width: 20px;
            height: 20px;
            margin-right: 8px;
        ...
        .dropdown_products_content li a {
            color: #64748b;
            text-decoration: none;
        }

        .dropdown_products_content li a:hover {
            color: #1f2937;
        }
        ...
        @media (max-width: 768px) {
            nav.sidebar {
                width: 220px;
                /* Giảm chiều rộng cho màn hình nhỏ */
                font-size: 14px;
...
<body class="bg-[#f5f5f7] text-[#222222] text-sm leading-relaxed font-[Inter]" x-data="{ notificationDropdownOpen: false }">
    ...
    <script>
        (function() {
            ...
            var shop = null;
            try {
                user = el ? JSON.parse(el.getAttribute('data-user') || 'null') : null;
            } catch (e) {}
            try {
                shop = el ? JSON.parse(el.getAttribute('data-shop') || 'null') : null;
            } catch (e) {}
            window.Laravel = {
                user: user,
                shop: shop
    ...
    <div class="flex">
        <div class="sticky top-0 h-[calc(100vh-53px)]">
            <nav class="w-[299px] h-screen bg-white border-r border-gray-200 text-xs text-gray-500 select-none">
                <div class="flex items-center justify-center h-[52.67px] bg-white border-b">
                    <img alt="Logo" class="w-[90px] object-cover" src="{{ asset('images/logo.png') }}" />
                </div>
                <div class="px-4">
                    <div class="sidebar mt-5 flex flex-col gap-1">
                        <div class="flex flex-col jusstify-center gap-3 px-[24px] mb-6">
                            <a href="{{ route('seller.dashboard') }}"
                                class="p-0 flex items-center justify-between cursor-pointer">
                                <span class="text-[18px] text-black font-bold">
                                    Thống Kê
                                </span>
                            </a>
                            <div class="border-b w-[202px]"></div>
                        </div>
                        ...
                        <div x-data="{ open: false }" class="">
                            ...
                            <ul x-show="open" x-transition
                                ...
                                <li class="text-sm text-[#64748b]">
                                    <a href="{{ route('seller.order.index') }}" class="link_admin">Danh Sách Đơn
                                        Hàng</a>
                                </li>
                            </ul>
                        </div>
                        ...
                        <div x-data="{ open: false }" class="">
                            ...
                            <ul x-show="open" x-transition
                                class="dropdown_products_content flex flex-col gap-2 py-[10px]">
                                <li class="text-sm text-[#64748b]">
                                    <a href="{{ route('seller.products.index') }}" class="link_admin">Danh sách Sản
                                        Phẩm</a>
                                </li>
                                <li class="text-sm text-[#64748b]">
                        ...
                        <div x-data="{ open: false }" class="">
                            ...
                            <ul x-show="open" x-transition
                                class="dropdown_products_content flex flex-col gap-2 py-[10px]">
                                <li class="text-sm text-[#64748b]">
                                    <a href="{{ route('seller.ads_campaigns.index') }}" class="link_admin">Quản Lý
                                        Quảng Cáo</a>
                                </li>
                                <li class="text-sm text-[#64748b]">
                                    <a href="{{ route('seller.ad_click_stats.index') }}" class="link_admin">Quản Lý
                                        Click</a>
                                </li>
                            </ul>
                        </div>
                        ...
                        <div x-data="{ open: false }" class="">
                            ...
                            <ul x-show="open" x-transition
                                ...
                                <li class="text-sm text-[#64748b]">
                                    <a href="{{ route('seller.reviews.index') }}" class="link_admin">
                                        Đánh Giá Khách Hàng
                                    </a>
                                </li>
                    ...
                    <div class="fixed bottom-10 flex flex-col jusstify-center gap-3 px-[24px]">
                        <a href="{{ route('seller.settings') }}"
                            class="text-[18px] p-0 flex items-center gap-1 text-black cursor-pointer">
                            <i class="fas fa-store mr-2"></i>Cài Đặt Cửa Hàng
                        </a>
                        <div class="border-b w-[202px]"></div>
                        <a href="/" class="text-[18px] flex itemx-center gap-2 text-black cursor-pointer"><i
                                class="fa-solid fa-arrow-left"></i>Quay Về Trang Chủ</a>
                    </div>
                </div>
            </nav>
        </div>
        <main class="flex-1 mx-auto">
            <header
                class="flex items-center justify-between px-[24px] py-2 border-b border-gray-200 bg-white sticky top-0 z-30 relative">
                <div class="">
                ...
                <div class="flex items-center space-x-6 text-gray-600">
                    ...
                    <div x-show="notificationDropdownOpen" x-transition:enter="transition ease-out duration-200"
                        ...
                        class="absolute top-[44px] right-[130px] mt-2 w-80 bg-white rounded-md shadow-lg overflow-hidden z-50"
                        @click.away="notificationDropdownOpen = false">
                        <div class="py-2">
                                ...
                                @forelse($groupedNotifications as $type => $notifications)
                                    ...
                                    <div class="notification-type" data-type="{{ $type }}">
                                        ...
                                        <div class="notification-items">
                                            ...
                                            @empty
                                                <div
                                                    class="px-4 py-3 text-center text-gray-500 border-b border-gray-100">
                                                    <p>Không có thông báo trong nhóm này</p>
                                                </div>
                                            @endforelse
                                        </div>
                                    </div>
                    ...
                    <div class="flex items-center space-x-2 cursor-pointer relative group" id="user-menu-trigger">
                        <img alt="User profile picture" class="w-8 h-8 rounded-full" height="32"
                            ...
                            width="32" />
                        <span
                            class="text-xs text-gray-700 select-none">{{ Auth::user()->username ?? 'Tài khoản' }}</span>
