@php
    ...
    $userId = Auth::id();
    $allOrders = Order::with(['items', 'address'])
        ->where('userID', $userId)
        ->orderBy('created_at', 'desc')
        ->get();
    $processingOrders = $allOrders->where('order_status', 'processing');
    $pendingOrders = $allOrders->where('order_status', 'pending');
    $shippedOrders = $allOrders->where('order_status', 'shipped');
    $deliveredOrders = $allOrders->where('order_status', 'delivered');
    $cancelledOrders = $allOrders->where('order_status', 'cancelled');
...
@section('content')
    <div class="container mx-auto py-5" style="min-height:80vh; background:#fff;">
        <div class="">
            <!-- Breadcrumbs -->
            <div class="flex flex-wrap items-center gap-2 mb-10 px-4 sm:px-0 md:mb-20 text-sm md:text-base">
            ...
            <div class="tab-content" id="orderStatusTabsContent">
                ...
                <div class="tab-pane" id="all" role="tabpanel" aria-labelledby="all-tab">
                    @forelse ($allOrders as $order)
                        <div class="order-block bg-white shadow-sm rounded-lg mb-6">
                            <div class="flex items-center justify-between py-4 px-4 sm:px-6 border-b border-gray-200">
                                <div class="flex items-center">
                                    <h6 class="font-bold text-base sm:text-lg mr-3 mb-0">Shop
                                        {{ $order->shop ? $order->shop->name : 'Unknown Shop' }}</h6>
                                    <button
                                    ...
                                    <div
                                        ...
                                        <div class="flex items-center">
                                            ...
                                            <div class="flex flex-col gap-2">
                                                <h6 class="font-normal text-sm sm:text-base mb-0">{{ $item->product_name }}
                                                </h6>
                                                <div class="text-gray-500 text-xs sm:text-sm">
                                                    <p>Số lượng: {{ $item->quantity }}</p>
                                                    <p>Màu sắc: {{ $item->color ?? 'N/A' }}</p>
                                                    <p>Kích thước: {{ $item->size ?? 'N/A' }}</p>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="flex items-center pr-4">
                                            <span class="font-bold text-sm sm:text-base text-black">Giá: <span
                                                    class="text-red-500">{{ number_format($item->unit_price, 0, ',', '.') }}đ</span></span>
                                        </div>
                                    </div>
                                @empty
                            ...
                            <div class="order-footer bg-gray-50 py-3 px-4 sm:px-6 flex justify-end items-center">
                                ...
                                @elseif ($order->order_status === 'cancelled' || $order->order_status === 'refunded')
                                    <!-- Không hiển thị "Mua Lại" hoặc "Liên hệ người bán" -->
                                @else
                                    <button
                                        class="border border-gray-500 text-gray-700 px-2 sm:px-3 py-1 sm:py-2 text-xs sm:text-sm mr-2 hover:bg-black hover:text-white">Liên
                                        hệ người bán</button>
