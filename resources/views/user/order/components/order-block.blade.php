...
<div class="order-block bg-white shadow-lg rounded-xl mb-6 border border-gray-100 hover:shadow-xl transition-all duration-300">
    <div class="flex items-center justify-between py-5 px-6 border-b border-gray-100 bg-gradient-to-r from-gray-50 to-white">
        ...
        <span class="inline-flex items-center rounded-full px-3 py-1.5 text-xs font-semibold ring-1 ring-inset {{ $order->status_classes }}">
    ...
    <div class="order-body px-6 py-5 {{ $order->status === 'cancelled' ? 'filter grayscale opacity-75' : '' }}">
        @forelse ($order->items as $item)
            <div class="product-row flex justify-between items-center py-4 @if(!$loop->last) border-b border-gray-100 @endif">
                <div class="flex items-center flex-1">
                    <div class="w-[100px] h-[100px] sm:w-[120px] sm:h-[120px] bg-gray-50 rounded-lg flex items-center justify-center mr-4 overflow-hidden border border-gray-200 {{ $order->order_status === 'cancelled' ? 'opacity-50' : '' }}">
                        @php
                    ...
                    <div class="flex flex-col gap-2 flex-1">
                        <h6 class="font-semibold text-sm sm:text-base mb-0 text-gray-800 {{ $order->status === 'cancelled' ? 'text-gray-400' : '' }}">
                            {{ $item->product_name ?? $item->variant->variant_name }}
                        </h6>
                        <div class="text-xs sm:text-sm {{ $order->status === 'cancelled' ? 'text-gray-400' : 'text-gray-500' }}">
                ...
                <div class="flex items-center pr-4">
                    <span class="font-bold text-sm sm:text-base flex items-center gap-2 {{ $order->status === 'cancelled' ? 'text-gray-400' : 'text-black' }}">
                        <span class="font-thin line-through {{ $order->status === 'cancelled' ? 'text-gray-300' : 'text-gray-400' }}">
                            {{ number_format($item->product->price ?? 0, 0, ',', '.') }}đ
                        </span>
                        <span class="font-semibold {{ $order->status === 'cancelled' ? 'text-gray-400' : 'text-red-600' }}">
            ...
            @if ($order->status === 'completed' && !in_array($item->productID, $reviewedProductIds))
                <div class="text-right mt-3 mb-2">
                    <button class="open-review-modal bg-gradient-to-r from-red-500 to-red-600 text-white px-4 py-2 text-xs sm:text-sm hover:from-red-600 hover:to-red-700 rounded-lg transition-all duration-200 shadow-md hover:shadow-lg font-medium"
                        data-product-id="{{ $item->productID }}" 
                        data-product-name="{{ $item->product_name }}"
                        data-order-id="{{ $order->id }}" 
                        data-shop-id="{{ $item->shopOrder->shopID }}">
    ...
    <div class="order-footer bg-gradient-to-r from-gray-50 to-gray-100 py-4 px-6 flex flex-wrap justify-between items-center rounded-b-xl">
        <div class="flex items-center mb-3 sm:mb-0">
            <span class="font-bold text-sm sm:text-base text-gray-700">
                Thành tiền:
                <span class="text-red-600 ml-1">{{ number_format($order->order->total_price ?? $order->total_price , 0, ',', '.') }}đ</span>
            </span>
        </div>
        ...
        <div class="flex flex-wrap gap-2">
            ...
            @if ($order->status === 'delivered')
                <button class="open-refund-modal bg-gradient-to-r from-orange-500 to-orange-600 text-white px-4 py-2 text-xs sm:text-sm hover:from-orange-600 hover:to-orange-700 rounded-lg transition-all duration-200 shadow-md hover:shadow-lg font-medium"
                    data-order-id="{{ $order->id }}">
                    <svg class="w-4 h-4 inline mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <button class="confirm-received bg-gradient-to-r from-green-500 to-green-600 text-white px-4 py-2 text-xs sm:text-sm hover:from-green-600 hover:to-green-700 rounded-lg transition-all duration-200 shadow-md hover:shadow-lg font-medium"
                    data-order-id="{{ $order->id }}">
                    <svg class="w-4 h-4 inline mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
