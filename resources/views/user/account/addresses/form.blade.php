<div class="grid grid-cols-1 gap-4">
    ...
    <input name="ward" value="{{ old('ward', $address->ward ?? '') }}" placeholder="Phường/Xã" required
        class="border rounded px-3 py-2" />
    <input name="district" value="{{ old('district', $address->district ?? '') }}" placeholder="Quận/Huyện" required
        class="border rounded px-3 py-2" />
    <input name="province" value="{{ old('province', $address->province ?? '') }}" placeholder="Tỉnh/Thành" required
        class="border rounded px-3 py-2" />
    <input name="zip_code" value="{{ old('zip_code', $address->zip_code ?? '') }}" placeholder="Mã bưu điện" required
        class="border rounded px-3 py-2" />
    ...
    <select name="address_type" class="border rounded px-3 py-2">
        ...
        <option value="office" {{ old('address_type', $address->address_type ?? '') === 'office' ? 'selected' : '' }}>
            Công ty</option>
        <option value="other" {{ old('addr  ess_type', $address->address_type ?? '') === 'other' ? 'selected' : '' }}>
            Khác</option>
    </select>



    <label><input type="checkbox" name="is_default"
            {{ old('is_default', $address->is_default ?? false) ? 'checked' : '' }}> Đặt làm mặc định</label>
