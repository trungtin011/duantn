@php
    $isChecked = in_array($node->id, request('category', []));
    $count = $node->products_count ?? $node->product_count ?? 0;
    $children = (isset($node->subCategories) && $node->subCategories) ? $node->subCategories : collect();

    // Recursive visibility check: show node if it or any descendant has products
    $hasVisibleCategory = function($n) use (&$hasVisibleCategory) {
        $cnt = $n->products_count ?? $n->product_count ?? 0;
        $kids = (isset($n->subCategories) && $n->subCategories) ? $n->subCategories : collect();
        if ($cnt > 0) return true;
        foreach ($kids as $k) {
            if ($hasVisibleCategory($k)) return true;
        }
        return false;
    };

    // Determine if this node should be shown
    $shouldShow = $isChecked || !request('query') || $hasVisibleCategory($node);

    // Filter children based on visibility (keep checked ones even if zero)
    $filteredChildren = $children->filter(function($c) use ($hasVisibleCategory) {
        $checked = in_array($c->id, request('category', []));
        return $checked || $hasVisibleCategory($c);
    });
...
<li>
    <div class="flex items-start">
        ...
        <label class="flex items-center w-full cursor-pointer p-2 rounded-md hover:bg-gray-50 transition-colors duration-200 {{ $isChecked ? 'bg-gray-100 border border-gray-200' : '' }}">
            <input type="checkbox" class="filter-checkbox mr-3" name="category[]" value="{{ $node->id }}" {{ $isChecked ? 'checked' : '' }}>
    ...
    @if($hasChildren)
        <ul class="ml-5 border-l pl-3 mt-1 space-y-1 {{ $level < 1 ? '' : 'hidden' }}">
            @foreach($filteredChildren as $child)
                @include('partials.category_tree_node', ['node' => $child, 'level' => $level + 1])
            @endforeach
        </ul>
    @endif
</li>
